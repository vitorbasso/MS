...
#define STOP     9170                       /* number of time intervals       */
#define sqr(x)   ((x) * (x))
  ...
  int main(void)
{
  ...
  while (index < STOP) {
    ...
    inventory     -= demand;
    if(count%35==0){
    inventarioMedio= (sum.holding/index) - (sum.shortage/index);
    printf("Indice %d: %6.2f\n",index,inventarioMedio);
    }
  }
  ...
  printf("  Nivel medio de inventario . = %6.2f\n",inventarioMedio);
