  ...
  int main(void)
{
  ...
  double p[26]     = {0.0};               /* probability estimates  */
  ...
  for (j = 0; j < 10000; j++) {         /* j conta as bolas */
        i = Equilikely(1, n);           /* gera uma caixa aleatoria  */
        xi[i]=xi[i]+1;                   /* armazena a bola na caixa */
   }

   for(i=0;i<=25;i++)                    //zera o vetor p[10]
    p[i]=0;

//ESTATISTICAS
//Calculo frequencia

    for(i=0;i<=25;i++){              //percorre o vetor analisando quantas vezes o numero de bolas repete em cada caixa
        for(j=0;j<1000;j++){
        if (xi[j]==i)
            p[i]=p[i] +1;
