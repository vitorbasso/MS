...
int main(){
    int i = 1, a = 7, m = pow(2, 31) -1, count = 0;
    long long x = a;
    ...
    while(count <10){
        ...
        int smallestValue = m;
        if(isPrime(value)){
            printf("prime modulus m: %d -", value);
            count++;
            for(a = 1; a < value; a++){
                if(isFullPeriod(a, value)){
                    if(a < smallestValue)
                        smallestValue = a;
                    inner_count++;
                }
            }
            printf("- number of full period multipliers: %d -- smallest one: %d\n\n", inner_count, smallestValue);
        }
    }
