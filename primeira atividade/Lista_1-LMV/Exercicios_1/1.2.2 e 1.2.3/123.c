...
#define SYSTEMATIC_INCREASE          1.00
#define L_TIME                       400
...
int main(void){
  ...
  while (!feof(fp)) {
    ...
    if(arrival <= L_TIME && departure > L_TIME) {
      number_jobs++;
    }
  ...
  // printf("%6.3f\n", sum.l_barra);
  printf("%6.2f\n", sum.q_barra);
  printf("Maximo Delay: %6.2f\n", max_delay);
  printf("Number of jobs at instant %ld: %ld\n", L_TIME, number_jobs);
  printf("Proportion of job delayed: %6.3f\n", ( (float) number_jobs_delayed)/ ( (float) index));
  // printf("%6.3f\n", sum.x_barra);
  fclose(fp);
  return (0);
}
